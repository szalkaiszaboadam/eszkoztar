...
export default function Cell({ id, isHeader, label, onNavigate }: CellProps) {
  ...
  return (
    <div
      ref={divRef}
      data-cell={id}
      tabIndex={0}                          // ← mindig 0, nem dinamikus!
      className={`border-b border-r border-gray-200 relative min-h-[28px] focus:outline-none ${
        isSelected ? "ring-2 ring-blue-500 ring-inset z-10" : ""  // ← ring helyett outline
      }`}
      onClick={handleClick}
    ...
    >
      {editing ? (
        <input
          ...
          className="absolute inset-0 w-full h-full px-1.5 text-sm outline-none bg-white border-2 border-blue-500 z-20"
        />
      ) : (
        <span
