...
export function processAndCrop(img: HTMLImageElement, originalIndex: number): CroppedImage {
  ...
  if (!found) {
    return { canvas: tempCanvas, ar: 1, originalIndex, cropOffsetX: 0, cropOffsetY: 0, cropW: img.width, cropH: img.height };
  }
...
export function renderToCanvas(canvas: HTMLCanvasElement, layout: AutoLayout, bgColor = "#ffffff") {
  ...
  const maxDraw = 2000 - externalMargin * 2;
  const scale = Math.min(maxDraw / totalW, maxDraw / totalH);
  const offX = (2000 - totalW * scale) / 2;
  const offY = (2000 - totalH * scale) / 2;
