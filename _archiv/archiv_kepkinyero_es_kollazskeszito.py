...
def termek_letolto_fázis1(page, url, kategoria_utvonal, letoltendo_db, eloszlas_mod, progress_file,
    ...
    mappa_path = os.path.join("../kepkinyero/Kollazs_Kepek", *tiszta_utvonal)
...
def interaktiv_kollazs_fázis2(ctx: Context, image_map_full, collage_progress_file):
    ...
    try:
        ...
        for kat_id, fajl_list in image_map_full.items():
            ...
            current_download_dir[0] = os.path.join("../kepkinyero/Kollazs_Kepek", *[tiszta_nev(p) for p in kat_utvonal])
...
if __name__ == "__main__":
    ...
    db_input = input("Max hány képet letölteni alap kategóriánként? (Alap: 10): ").strip()
    ...
    kztes_v = input("\nSzeretnél summary (összesítő) kollázsokat is a főkategóriákról? (i/n): ").strip().lower()
    letolt_koztes = True if kztes_v == 'i' else False
    ...
    with sync_playwright() as p:
        # A --start-maximized argumentum indítja teljes képernyőn a böngészőt
        ...
        if ctx:
            ...
            letoltes_szukseges = (len(retry_list) > 0) or not os.path.exists(scr_prog_f) or (
                        not befejezett_kategoriak and not retry_list)
